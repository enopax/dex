...
func (c *Connector) RegisterHandlers(mux *http.ServeMux) {
	...
	mux.HandleFunc("/setup-auth/password", c.handlePasswordSetup)
}
...
// AuthSetupToken represents a token for authentication setup.
// This token is generated by the Platform when a user completes registration
// and needs to set up their authentication methods.
type AuthSetupToken struct {
	// Token is the unique token identifier
	Token string `json:"token"`

	// UserID is the user who is setting up auth
	UserID string `json:"user_id"`

	// Email is the user's email
	Email string `json:"email"`

	// CreatedAt is when token was created
	CreatedAt time.Time `json:"created_at"`

	// ExpiresAt is when token expires
	ExpiresAt time.Time `json:"expires_at"`

	// Used indicates whether token has been used
	Used bool `json:"used"`

	// UsedAt is when token was used
	UsedAt *time.Time `json:"used_at,omitempty"`

	// ReturnURL is the URL to return to after setup
	ReturnURL string `json:"return_url"`
}

// Validate checks if the token is valid.
func (t *AuthSetupToken) Validate() error {
	if t.Token == "" {
