...
func TestDefaultTestConfig(t *testing.T) {
	...
	assert.Equal(t, "http://localhost:5556", config.BaseURL)
	assert.Equal(t, "Enopax Test", config.Passkey.RPName)
	assert.Equal(t, "localhost", config.Passkey.RPID)
	assert.Equal(t, []string{"http://localhost:5556"}, config.Passkey.RPOrigins)
	assert.Equal(t, "preferred", config.Passkey.UserVerification)
	assert.Equal(t, 600, config.MagicLink.TTL)
