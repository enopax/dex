...
func TestFileStorageSpecific(t *testing.T) {
	t.Run("DirectoryCreation", func(t *testing.T) {
		...
		for _, dir := range expectedDirs {
			...
			if info.Mode().Perm() != 0o700 {
				t.Errorf("directory %s has wrong permissions: got %o, want 0o700", dir, info.Mode().Perm())
			}
		}
	})
	...
	t.Run("FilePermissions", func(t *testing.T) {
		...
		if info.Mode().Perm() != 0o600 {
			t.Errorf("password file has wrong permissions: got %o, want 0o600", info.Mode().Perm())
		}
	})
